...
async fn main() -> anyhow::Result<()> {
    ...
    actorscript!(
        ...
        #[labels(
            ...
            sh24="GMT\nAGWS SH24",
            sh24_kern="AGWS SH24\nKernel",
            dint_ttc="Double Integrator\nTT controller",
            on_axis="On-axis\nGMT",
            sh48_wave="SH48 GMT\nWave-Sensor",
            sampler = "1:10",
            sh24_frame = "SH24\nframe")]
        1: timer[Tick] -> optical_state[OpticsState]
            -> sh24[Sh24Frame]! -> sh24_kern[M2RigidBodyMotions] -> dint_ttc[M2RigidBodyMotions] -> m2_state[M2State]
...

    {
        ...
        // // closed-loop calibration of M1 Sx RBMS with SH48
        // let file_name = "sh48_closed-loop_M1-RBM_calib.pkl";
        // let m1_rbm_recon: ClosedLoopReconstructor =
        //     if let Ok(m1_rbm_recon) = ClosedLoopReconstructor::from_data_repo(file_name) {
        //         m1_rbm_recon
        //     } else {
        //         let sh48_omb: OpticalModelBuilder<CameraBuilder<1>> =
        //             (&ShackHartmannBuilder::<Reconstructor>::sh48().use_calibration_src()).into();
        //         let sh24_omb: OpticalModelBuilder<CameraBuilder<1>> =
        //             (&ShackHartmannBuilder::<Reconstructor>::sh24().use_calibration_src()).into();
        //         let mut m1_rbm_recon =
        //             <CentroidsProcessing as ClosedLoopCalibration<GmtM1, Imaging>>::calibrate(
        //                 &(&sh48_omb).into(),
        //                 //CalibrationMode::r_xy(1e-6),
        //                 CalibrationMode::RBM([Some(1e-6), Some(1e-6), Some(1e-6), Some(1e-6), Some(1e-6), Some(1e-6)]),
        //                 &(&sh24_omb).into(),
        //                 CalibrationMode::r_xy(1e-6),
        //             )?;
        //         m1_rbm_recon.pseudoinverse().to_data_repo(file_name)?;
        //         m1_rbm_recon
        //     };
        ...
        actorscript!(
            ...
            1: timer[Tick] -> optical_state[OpticsState]
                -> sh24[Sh24Frame]! -> sh24_kern[M2RigidBodyMotions] -> dint_ttc[M2RigidBodyMotions] -> merge_agws[M2State]
                    -> optical_state[OpticsState] -> on_axis[WfeRms<-9>] -> print
